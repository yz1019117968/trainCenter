...
public partial class 培训动态查看 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            mycon.Open();
            string strselect = "select * from dbo.培训动态 where 标题='" + Session["mids"].ToString() + "'";
            SqlCommand sqlcmd = new SqlCommand(strselect, mycon);
            SqlDataReader dr = sqlcmd.ExecuteReader();
            if (dr.Read())
            {
                Label1.Text = dr["标题"].ToString();
                Label2.Text = "作者：" + dr["作者"].ToString() + "&nbsp&nbsp编辑：" + dr["编辑"].ToString() + "&nbsp&nbsp日期：" + dr["日期"].ToString();
        ...
        {
            Label l1 = new Label();
            l1.Text = "&nbsp&nbsp文件" + (i + 1).ToString() + "：";
            l1.Font.Name = "微软雅黑";
