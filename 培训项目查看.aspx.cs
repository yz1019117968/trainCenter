...
public partial class 培训项目查看 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            mycon.Open();
            string strselect = "select * from dbo.培训项目 where 项目标题='" + Session["mids"].ToString() + "'";
            SqlCommand sqlcmd = new SqlCommand(strselect, mycon);
            SqlDataReader dr = sqlcmd.ExecuteReader();
            if (dr.Read())
            {
                Label1.Text = dr["项目标题"].ToString();
                 Label2.Text= "项目负责人："+dr["项目负责人"].ToString() +"   日期："+dr["日期"].ToString();
                Label3.Text= dr["项目信息"].ToString();
